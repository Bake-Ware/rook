...
PiKVM through `mcp.example.com` like any other rook worker.
...
Below assumes PiKVM at `<pikvm-ip>` and `root:root` SSH credentials.
...
tar c rook -C ../.. telesthete | \
  ssh root@<pikvm-ip> 'mkdir -p /opt/rook-worker/src && tar x -C /opt/rook-worker/src'
```
...
```
scp pikvm/rook-worker.service root@<pikvm-ip>:/etc/systemd/system/
scp pikvm/rook-worker.env.example root@<pikvm-ip>:/etc/rook-worker.env
ssh root@<pikvm-ip> 'chmod 600 /etc/rook-worker.env'
```
...
```
ssh root@<pikvm-ip> 'systemctl daemon-reload && systemctl enable --now rook-worker'
ro
```

### 5. Verify on the band

Call `rook_workers` through `mcp.example.com` — `pikvm-rack1` (or your
