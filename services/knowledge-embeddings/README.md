...
Run it in a container with a persistent `/models` volume, two CPU threads, a
768 MiB limit, and only a loopback host port. If it runs on a different machine
from the hub, expose it on the hub's loopback with an SSH reverse forward rather
than on the public network.
