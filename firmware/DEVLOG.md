...
`https://mcp.example.com/mcp` without any LAN access to the dongle.
...
(default `hub.example.com`), `hub_port` (7474), `band_psk`,
`worker_name` (auto = `rook-kvm-<mac suffix>`), `phone_ssid`,
`phone_pass`. DNS record `hub.example.com → 203.0.113.10` lives in
