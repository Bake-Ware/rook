...
function render(){
 ...
 ${b.workers.map(w=>`<div class="worker-line"><span>${esc(w.name)}${w.description?`<small>${esc(w.description)}</small>`:''}<small>${w.online?'Online':'Offline'}${w.can_move?'':' · update required to move'}</small></span>${owner?`<button class="secondary" data-op="move" data-band="${esc(b.id)}" data-worker="${esc(w.id)}" ${!w.online||!w.can_move||open||!b.active?'disabled':''}>Move to band…</button>`:''}</div>`).join('')}
